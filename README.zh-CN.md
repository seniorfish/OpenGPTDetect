...
下图展示中文文本的分析效果。图中文字的**前半部分为人工撰写，后半部分由 AI 续写**。

![编辑器 · 中文示例：前半（人写）以红/橙为主，后半（AI 续写）明显偏绿](https://github.com/user-attachments/assets/34ee5037-3690-4720-b000-45087a3ce7a6)

**颜色含义**（逐 token 染色遵循 PPL 色阶，由红到绿连续过渡）：

- **红色 = 高 PPL**：模型对下一个 token 感到「意外」——**人写的**典型特征
- **绿色 = 低 PPL**：模型能轻松预测——**AI 生成**的典型特征

上图中，前半部分以红 / 橙为主，后半部分明显偏绿，人机边界清晰可辨。人工撰写的部分取自[腾讯 AI 检测助手](https://matrix.tencent.com/ai-detect/)的官方示例。

**编辑器特性**：颜色自定义、选区忽略、逐 token 分析等；针对中文、英文文本分别做了调优。
